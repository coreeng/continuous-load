...
func main() {
	...
	fs.Duration("http-client-response-header-timeout", 3*time.Second, "client response  header timeout duration")
	fs.Duration("http-client-idle-conn-timeout", 3*time.Second, "client idle conn timeout duration")
	fs.Bool("http-client-tls-insecure-skip-verify", false, "client tls insecure skip verify")
	fs.Duration("http-server-timeout", 30*time.Second, "server read and write timeout duration")
	fs.Duration("http-server-shutdown-timeout", 5*time.Second, "server graceful shutdown timeout duration")
	fs.Duration("http-server-shutdown-grace-period", 5*time.Second, "server graceful shutdown grace period duration")
